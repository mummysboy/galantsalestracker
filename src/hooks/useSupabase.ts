...
import { supabaseService, SalesRecord, CustomerProgression } from '../services/supabase';
...
export const useSupabase = (): UseSupabaseReturn => {
  ...
  const saveSalesRecord = useCallback(async (record: Omit<SalesRecord, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      setLoading(true);
      setError(null);
      const savedRecord = await supabaseService.saveSalesRecord(record);
  ...
  const saveSalesRecords = useCallback(async (records: Omit<SalesRecord, 'id' | 'createdAt' | 'updatedAt'>[]) => {
    try {
      setLoading(true);
      setError(null);
      const savedRecords = await supabaseService.saveSalesRecordsWithDedup(records);
  ...
  const saveSalesRecordsFast = useCallback(async (records: Omit<SalesRecord, 'id' | 'createdAt' | 'updatedAt'>[]) => {
    try {
      setLoading(true);
      setError(null);
      const savedRecords = await supabaseService.saveSalesRecordsFast(records);
      setSalesRecords(prev => [...prev, ...savedRecords]);
      return savedRecords;
    } catch (err) {
  ...
  const loadSalesRecordsByPeriod = useCallback(async (period: string) => {
    try {
      setLoading(true);
      setError(null);
      const records = await supabaseService.getSalesRecordsByPeriod(period);
      setSalesRecords(records);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load sales records');
    } finally {
  ...
  const loadAllSalesRecords = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const records = await supabaseService.getAllSalesRecords();
      setSalesRecords(records);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load sales records');
    } finally {
  ...
  const saveCustomerProgression = useCallback(async (distributor: string, customerName: string, progression: any) => {
    try {
      setLoading(true);
      setError(null);
      const savedProgression = await supabaseService.saveCustomerProgression(distributor, customerName, progression);
  ...
  const saveCustomerProgressionWithDedup = useCallback(async (distributor: string, customerName: string, progression: any) => {
    try {
      setLoading(true);
      setError(null);
      const savedProgression = await supabaseService.saveCustomerProgressionWithDedup(distributor, customerName, progression);
      setCustomerProgressions(prev => {
        const newMap = new Map(prev);
        newMap.set(`${distributor}-${customerName}`, savedProgression);
        return newMap;
  ...
  const loadCustomerProgressionsByDistributor = useCallback(async (distributor: string) => {
    try {
      setLoading(true);
      setError(null);
      const progressions = await supabaseService.getCustomerProgressionsByDistributor(distributor);
      setCustomerProgressions(prev => {
        const newMap = new Map(prev);
  ...
  const saveAppState = useCallback(async (key: string, value: any) => {
    try {
      setLoading(true);
      setError(null);
      await supabaseService.saveAppState(key, value);
      setAppState(prev => {
        const newMap = new Map(prev);
  ...
  const loadAppState = useCallback(async (key: string) => {
    try {
      setLoading(true);
      setError(null);
      const state = await supabaseService.getAppState(key);
      if (state) {
        setAppState(prev => {
          const newMap = new Map(prev);
  ...
  const loadAllAppStates = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const states = await supabaseService.getAllAppStates();
      setAppState(prev => {
        const newMap = new Map(prev);
  ...
  const clearDistributorData = useCallback(async (distributor: string) => {
    try {
      setLoading(true);
      setError(null);
      await supabaseService.clearDistributorData(distributor);
