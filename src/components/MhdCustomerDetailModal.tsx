...
function calculateRetailerCustomerDataAllPeriods(mhdData: AlpineSalesRecord[], retailerName: string, viewMode: 'month' | 'quarter' = 'month') {
  const retailerRecords = mhdData.filter(record => record.customerName === retailerName);
  ...
  retailerRecords.forEach(record => {
    // Use accountName directly as customer name (Level 2)
...
function calculateCustomerProductDataAllPeriods(mhdData: AlpineSalesRecord[], retailerName: string, customerName: string, viewMode: 'month' | 'quarter' = 'month') {
  ...
  customerRecords.forEach(record => {
    // Use productName directly as the product identifier (Level 3)
...
const MhdCustomerDetailModal: React.FC<MhdCustomerDetailModalProps> = ({
  ...
  // Show customers (Level 2) summary for retailer with visible periods only
  const { customers, periods: allPeriods } = calculateRetailerCustomerDataAllPeriods(mhdData, retailerName, viewMode);
  
  // Filter periods to only show visible ones
  const periods = visiblePeriods;
  
  // Debug: Log what we found
  console.log('MHD Modal Debug:', {
    retailerName,
    totalRecords: mhdData.length,
    retailerRecords: mhdData.filter(r => r.customerName === retailerName).length,
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {customers.length > 0 ? (
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left px-4 py-3 text-sm font-bold text-gray-900">Location</th>
                      {periods.map((period) => (
                  ...
                  <tbody>
                    {customers.map((customer, index) => {
                      ...
                      const { products } = calculateCustomerProductDataAllPeriods(mhdData, retailerName, customer.customerName, viewMode);
            ...
            <div className="text-center py-8 text-gray-500 px-6">
              ...
              <p className="text-sm mt-2">Location data may not be populated in the uploaded data</p>
            </div>
          )}
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-600">
                Sum of Cases by Location • Click locations to view product breakdown
              </div>
              <div className="flex items-center gap-2">
                <button
