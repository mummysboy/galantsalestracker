...
// Calculate product data for a customer with all periods
function calculateCustomerProductDataAllPeriods(troiaData: AlpineSalesRecord[], customerName: string, viewMode: 'month' | 'quarter' = 'month') {
  const customerRecords = troiaData.filter(record => record.customerName === customerName);
  
  // Get all unique products for this customer
  const productsMap = new Map<string, Map<string, number>>();
  const productMetadata = new Map<string, { productCode?: string }>();
...
interface TroiaCustomerDetailModalProps {
  customerName: string;
...
const TroiaCustomerDetailModal: React.FC<TroiaCustomerDetailModalProps> = ({
  customerName,
  troiaData,
  isOpen,
  onClose,
  ...
  const availablePeriods = useMemo(() => {
    ...
    return periods;
  }, [troiaData, customerName, viewMode]);
  ...
  // Show products for this customer with visible periods only
  const { products } = calculateCustomerProductDataAllPeriods(troiaData, customerName, viewMode);
      ...
      >
        ...
        <div className="p-6 pb-0">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900">{customerName} • Product Sales</h3>
              <Button
                variant="outline"
                size="sm"
                onClick={onClose}
          ...
          <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-600">
                Sum of Cases by Product • Customer: {customerName}
              </div>
...
export default TroiaCustomerDetailModal;
